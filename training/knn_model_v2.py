...
class OrganizationCategorizer:
    ...
    def __init__(self, k=3, similarity_threshold=0.1):
        """
        Initialize the categorizer
           k: Number of nearest neighbors to consider (optimal: 3)
    ...
    def fit(self, df, analyzer="word", ngram_range=(1, 1), min_df=1, max_df=0.8, sublinear_tf=True):
        """
        Train the model on the provided dataset with optimal hyperparameters
           df columns: ['name_org', 'oc_id', 'Business', 'Group', 'Industry']
           Optimal params: analyzer='word', ngram_range=(1,1), min_df=1, max_df=0.8, sublinear_tf=True
        """
...
def main(k_val=5, analyzer="word", ngram_range=(1, 3), min_df=2, max_df=0.9, sublinear_tf=True):
    ...
    # split by concept to avoid concept leakage between train and test sets
    concept_df = df.groupby('oc_id').first()[['Industry']].reset_index()

    # keep only industries with at least 2 concepts (required for stratified concept split)
    ...
    # stratified split on unique concepts, then map back to organization rows
    ...
    concept_overlap = set(train_concepts) & set(test_concepts)
    ...
    print(f"Concept overlap between train/test: {len(concept_overlap)}")
    ...
    # save model
    model.save_model("../model/org_categorizer_knn_model_v2.pkl")
